// fetch-ticker-cape proxies to Yahoo Finance under the project's own
...
describe("fetch-ticker-cape auth gate", () => {
  it("rejects a request with no Authorization header", async () => {
    const res = await handler(
      new Request("https://example.com/fetch-ticker-cape", {
        method: "POST",
  ...
  it("lets an OPTIONS preflight through without an auth check", async () => {
    const res = await handler(new Request("https://example.com/fetch-ticker-cape", { method: "OPTIONS" }));
