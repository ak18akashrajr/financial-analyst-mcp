// fetch-benchmark-prices writes to benchmark_history via the service-role
...
describe("fetch-benchmark-prices auth gate", () => {
  it("rejects a request with no Authorization header", async () => {
    const res = await handler(
      new Request("https://example.com/fetch-benchmark-prices", {
        method: "POST",
  ...
  it("lets an OPTIONS preflight through without an auth check", async () => {
    const res = await handler(new Request("https://example.com/fetch-benchmark-prices", { method: "OPTIONS" }));
