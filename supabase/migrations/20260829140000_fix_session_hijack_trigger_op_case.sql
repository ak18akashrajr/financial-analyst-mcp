-- Fix: detect_session_hijack() compared the raw TG_OP special variable against
-- lowercase 'insert'/'update'/'delete' literals when deciding whether to
-- capture old_values/new_values. TG_OP is always uppercase in Postgres
-- ('INSERT'/'UPDATE'/'DELETE'), so neither branch ever matched — every
-- incident row was being recorded with old_values and new_values both null,
-- losing the before/after diff entirely, even though the incident itself was
-- still correctly detected (session_id/table/operation/ip/user_agent were all
-- unaffected, since those come from other, correctly-cased sources).
--
-- Caught empirically: a real update-path replay test on cash_settings produced
-- an incident row with old_values/new_values both null despite being an
-- UPDATE. Fixed by comparing against v_operation (already lower(TG_OP)),
-- which the function computes earlier for the `operation` column and should
-- have reused here.
  ...
  if not found then
    ...
    if v_last_ip is not null and v_ip is not null and v_last_ip <> v_ip then
      ...
      values (
        ...
        v_operation,
        v_row_id,
        case when v_operation in ('update', 'delete') then to_jsonb(OLD) else null end,
        case when v_operation in ('insert', 'update') then to_jsonb(NEW) else null end,
        v_ip,
        v_user_agent
      );
    end if;
